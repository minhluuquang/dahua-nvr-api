...
## Repo Layout
- src/index.ts: Hono app, OpenAPI schemas, routes, handlers.
- src/dahua-client.ts: Dahua RPC client and auth flow.
...
- Typecheck only: `npm run build` (no separate typecheck script).
...
- Keep schema constants in the same file as the routes they serve.
...
- Add `createRoute` definition and `app.openapi` handler.
- Update the health endpoint list if relevant.
- Update docs in `docs/README.md`.
- Verify OpenAPI JSON renders in `/doc` and `/swagger`.
...
## Tips for Agents
- Run `npm run dev` for local iteration.
